...
export const spreadNS = GC.Spread.Sheets;
...
export function mergeCells(sheet) {
    var sels = sheet.getSelections();
    var rowCount = sheet.getRowCount(),
        columnCount = sheet.getColumnCount();
...
function adjustSpreadSize() {
    var height = $("#inner-content-container").height() - $("#formulaBar").height() - MARGIN_BOTTOM,
        spreadHeight = $(g_spreadDiv).height();

    if (spreadHeight !== height) {
        $("#controlPanel").height(height);
        $(g_spreadDiv).height(height);
        spread.refresh()
    }
}
...
function getTabItem(tabName) {
    return $(".insp-container ul.nav-tabs a[href='#" + tabName + "TabContent']").parent();
}
...
function resetPieColor($colorSpanDiv) {
    ...
    var $colorDiv = $colorSpanDiv.parent().parent().parent().parent().parent();
    $colorDiv.remove();
    var $pieSparklineColorContainer = $("#pieSparklineColorContainer");
    var colorArray = [];
...
export function initSpread(spreadDiv,formulaboxDiv,param,dispatch) {
    g_spreadDiv = spreadDiv
    g_dispatch = dispatch
    function localizeUI() {
    ...
    spread = new spreadNS.Workbook($(spreadDiv)[0], param);
    excelIO = new GC.Spread.Excel.IO();


    getThemeColor();
    fbx = new spreadNS.FormulaTextBox.FormulaTextBox(document.getElementById(formulaboxDiv));
    fbx.workbook(spread);
    ...
    g_dispatch(InitSpreadAction(spread))
    return spread
};


function getHitTest(pageX, pageY, sheet) {
    var offset = $(g_spreadDiv).offset(),
        x = pageX - offset.left,
        y = pageY - offset.top;
...
function exportToJSON() {
    ...
    if (isSafari) {
        ...
        window.saveAs(new Blob([text], {type: "text/plain;charset=utf-8"}), fileName + ".json");
    }
}
